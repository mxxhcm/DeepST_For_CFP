...
from deepst.datasets import TaxiRM_5_frame
from deepst.datasets import TaxiRM
import copy
...
def main():
    ...
    print("loading data...")
    ts = time.time()
    fname = os.path.join(DATAPATH, 'CACHE', 'TaxiRM_C{}_P{}_T{}_no_external.h5'.format(
    ...
    if os.path.exists(fname) and CACHEDATA:
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = TaxiRM.load_data(
...
def test():
    ...
    if os.path.exists(fname) and CACHEDATA:
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = TaxiRM_5_frame.load_data(
    ...
    rmse_rome_one = np.sqrt(np.square(predict_1 - Y_test_1)) * (mmn._max - mmn._min) / 2.
    rmse_rome_one = np.transpose(rmse_rome_one, [0, 2, 3, 1])
    np.savez("deepst_rome_one", rmse_rome_one)
    ...
    rmse_rome_two = np.sqrt((np.square(predict_1-Y_test_1) + np.square(predict_2-Y_test_2))/2)*(mmn._max - mmn._min) / 2.
    rmse_rome_two = np.transpose(rmse_rome_two, [0, 2, 3, 1])
    np.savez("deepst_rome_two", rmse_rome_two)
    ...
    rmse_rome_three = np.sqrt((np.square(predict_1 - Y_test_1) + np.square(predict_2 - Y_test_2) + np.square(predict_3-Y_test_3))/3)*(mmn._max - mmn._min) / 2.
    rmse_rome_three = np.transpose(rmse_rome_three, [0, 2, 3, 1])
    np.savez("deepst_rome_three", rmse_rome_three)
    ...
    rmse_rome_four = np.sqrt((np.square(predict_1 - Y_test_1) + np.square(predict_2 - Y_test_2)
                            + np.square(predict_3-Y_test_3)+np.square(predict_4 - Y_test_4))/4)*(mmn._max - mmn._min) / 2.
    rmse_rome_four = np.transpose(rmse_rome_four, [0, 2, 3, 1])
    np.savez("deepst_rome_four", rmse_rome_four)
    ...
    rmse_rome_five = np.sqrt((np.square(predict_1 - Y_test_1) + np.square(predict_2 - Y_test_2) + np.square(predict_3 - Y_test_3)
        +np.square(predict_4 - Y_test_4) +np.square(predict_5 - Y_test_5)) / 5) * (
            mmn._max - mmn._min) / 2.
    rmse_rome_five = np.transpose(rmse_rome_five, [0, 2, 3, 1])
    np.savez("deepst_rome_five", rmse_rome_five)
