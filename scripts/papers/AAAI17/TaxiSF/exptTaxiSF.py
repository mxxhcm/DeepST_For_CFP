...
from deepst.datasets import TaxiSF
...
T = 240  # number of time intervals in one day
...
def main():
    ...
    print("loading data...")
    ts = time.time()
    fname = os.path.join(DATAPATH, 'CACHE', 'TaxiSF_C{}_P{}_T{}_no_external.h5'.format(
    ...
    if os.path.exists(fname) and CACHEDATA:
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = TaxiSF.load_data(
    ...
    fname_param = os.path.join('MODEL', '{}.no_external.best.h5'.format(hyperparams_name))
    ...
    model.save_weights(os.path.join(
        'MODEL', '{}.no_external.h5'.format(hyperparams_name)), overwrite=True)
    pickle.dump((history.history), open(os.path.join(
        path_result, '{}.no_external.history.pkl'.format(hyperparams_name)), 'wb'))
    ...
    fname_param = os.path.join(
        'MODEL', '{}.no_external.cont.best.h5'.format(hyperparams_name))
    model_checkpoint = ModelCheckpoint(
        fname_param, monitor='rmse', verbose=0, save_best_only=True, mode='min')
    history = model.fit(X_train, Y_train, nb_epoch=nb_epoch_cont, verbose=1, batch_size=batch_size, callbacks=[
                        model_checkpoint])
    pickle.dump((history.history), open(os.path.join(
        path_result, '{}.no_external.cont.history.pkl'.format(hyperparams_name)), 'wb'))
    model.save_weights(os.path.join(
        'MODEL', '{}.no_external_cont.h5'.format(hyperparams_name)), overwrite=True)
